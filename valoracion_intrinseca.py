...
def _row_value_sum_last(df: pd.DataFrame, row_names: List[str], periods: int = 4) -> Optional[float]:
    """
...
def _last_row_value(df: pd.DataFrame, row_names: List[str]) -> Optional[float]:
    """
...
def get_company_data(ticker: str) -> Dict[str, Any]:
    ...
    revenue_ttm = _row_value_sum_last(q_is, ["Total Revenue", "Revenue"])
    net_income_ttm = _row_value_sum_last(q_is, ["Net Income", "Net income"])

    # EPS derivado si hace falta
    eps = trailing_eps
    if eps is None and (net_income_ttm is not None and shares):
        eps = safe_div(net_income_ttm, shares)

    # FCF TTM: buscar fila directa; si no, OCF - CapEx
    fcf_ttm = _row_value_sum_last(q_cf, ["Free Cash Flow", "Free cash flow"])
    if fcf_ttm is None:
    ...
    total_debt = _last_row_value(q_bs, ["Total Debt", "Short Long Term Debt", "Long Term Debt"])
    cash_eq = _last_row_value(q_bs, ["Cash And Cash Equivalents", "Cash And Short Term Investments", "Cash"])
    net_debt = None
...
# -----------------------------
def method_pe(eps: Optional[float], sector_pe: Optional[float]) -> Optional[float]:
    if eps is None or sector_pe is None or sector_pe <= 0:
        return None
    val = eps * sector_pe
    return val if val > 0 else None

def method_gordon(dps: Optional[float], r: float, g: float) -> Optional[float]:
...
def method_dcf(fcf_ttm: Optional[float],
...
def classify_vs_price(intrinsic: Optional[float], price: Optional[float], band: float = 0.10) -> Optional[str]:
...
def value_tickers(tickers: List[str], cfg: ValuationConfig) -> pd.DataFrame:
    rows = []
    for t in tickers:
        ...
        sector_pe = None
        if cfg.override_sector_pe and t in cfg.override_sector_pe:
            sector_pe = cfg.override_sector_pe[t]
        pe_val = method_pe(eps, sector_pe)

        # Gordon
        gordon_val = method_gordon(
            dps=data["dps_ttm"],
            r=cfg.discount_rate,
        ...
        classification = classify_vs_price(avg_val, data["price"], cfg.neutrality_band) if avg_val else None
